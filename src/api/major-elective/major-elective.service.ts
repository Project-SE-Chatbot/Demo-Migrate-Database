...
export class MajorElectiveService {
    ...
    constructor(
        @InjectRepository(Major_Elective) private majorElectiveRepository: Repository<Major_Elective>,
        @InjectRepository(Major_Elective_Key) private majorElectiveKeyRepository: Repository<Major_Elective_Key>
    ){}
    ...
    async findMajorElectiveByKey(findKey: string) {
        console.log(findKey);
        const findKey1 = await this.majorElectiveKeyRepository.find({where: {key_1: findKey}})
        const findKey2 = await this.majorElectiveKeyRepository.find({where: {key_2: findKey}})
        const findKey3 = await this.majorElectiveKeyRepository.find({where: {key_3: findKey}})
        const findKey4 = await this.majorElectiveKeyRepository.find({where: {key_4: findKey}})
        const findKey5 = await this.majorElectiveKeyRepository.find({where: {key_5: findKey}})
        const findKey6 = await this.majorElectiveKeyRepository.find({where: {key_6: findKey}})
